...
export function GlowButton({
  ...
  const classes = [
    "group relative inline-flex items-center justify-center overflow-hidden px-6 py-3 text-sm font-medium transition-all duration-300",
    ...
    <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }}>
      {href ? (
        <Link href={href} className={classes}>
          {!isPrimary && (
            <span className="absolute inset-y-0 left-0 w-0 bg-orange-50 transition-all duration-300 group-hover:w-full" />
          )}
